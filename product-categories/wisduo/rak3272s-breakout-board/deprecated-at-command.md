---
title: RAK3272S WisDuo Breakout Board Datasheet Deprecated AT Command Manual
description: Deprecated AT Command
image: "https://images.docs.rakwireless.com/wisduo/rak3272s-breakout-board/rak3272s-breakout.png"
keywords:
  - RAK3272S  Breakout Board
  - wisduo
sidebar_label: Deprecated AT Command
slug: /product-categories/wisduo/rak3272s-breakout-board/deprecated-at-command/
download: true
---
...
This command allows the AT command input to be displayed on the Serial Terminal.
...
:::tip NOTE
Restart the module for the new baud rate to take effect.
:::
...
## Join and Send Data to LoRaWAN Network
...
|                                 | *Param2* = **Auto-Join Configuration**: Set to `1` to enable auto-join on power-up or `0` to disable it (default is `0`). |                                  |                         |
|                                 | *Param3* = **Reattempt interval**: 7 - 255 seconds (8 is default)                                 |                                  |                         |
|                                 | *Param4* = **Number of Join Attempts**: Acceptable range is 0–255 (default is `0`).                                      |                                  |                         |
...
If joining fails, ensure the device is within the gateway's coverage area. Verify that the RAK3172 is set to LoRaWAN mode using `AT+NWM=1`, the region is correctly configured with `AT+BAND`, and the EUIs and keys are accurate.
...
```
With a downlink from the Network Server:
Downlink Information: **FPort = 3**, **Payload = 4321**.
```
AT+SEND=2:1234
...
`AT+CFS=?` reflects the status of the last confirmed packet sent using the `AT+SEND` command. It returns `1` if the last confirmed packet was successful and `0` if it failed. This status remains accurate even if successful unconfirmed packets are sent between confirmed payloads. `AT+CFS=?` exclusively tracks the last confirmed packet.
...
When operating in **CLASS B**, additional return values are displayed to indicate the current state of Class B operation.
...
This command retrieves the duty cycle time (in seconds). It is applicable only to the **EU868**, **RU864**, and **EU433** frequency bands. Queries for other frequency bands will return `0`.
...
Input Parameter Details for Link Check:
- **0**: Disable Link Check.
- **1**: Execute Link Check once on the next payload uplink.
- **2**: Automatically execute a one-time Link Check after every payload uplink.
Reply format:
...
- **port**: Value range is 1 to 223.
- **confirm**: 0 for confirmed or 1 for unconfirmed.
- **nbtrials**: Number of retransmissions, value range is 0 to 7.
...
:::tip NOTE
The commands in these sections work in Class B mode. Configure your device to Class B via `AT+CLASS=B` to ensure that these commands will work.
:::
...
Description: Receive Signal Strength Indicator (RSSI).
...
Use the `AT+LTIME` command to get the exact time data/value.
:::
...
Another separate device should simultaneously execute the `AT+TTX=3` command to obtain accurate readings for `AT+TRX=3`.
```
AT+TRX=3
154s047:OnRxDone
...
You can only remove a group if its address has already been added.
```
AT+RMVMULC=11223344
