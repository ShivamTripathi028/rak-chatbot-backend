---
title: RAK3172 WisDuo LoRaWAN Module Quick Start Guide | Setup & Configuration
description: Set up the RAK3172 module, based on the STM32WLE5CCU6 chip, for LoRaWAN and LoRa P2P. Learn OTAA, ABP activation, UART configuration, and firmware updates.
keywords:
  - rak3172 
  - rak3172 setup guide 
  - lorawan module setup 
  - stm32wle5ccu6 lora module 
  - lorawan module guide
  - stm32wle5ccu6 
  ...
  - configure lora p2p 
  - lora module uart 
  - lorawan firmware updates
sidebar_label: Quick Start Guide
slug: /product-categories/wisduo/rak3172-module/quickstart/
date: 2021-04-12
download: true
---
...
Before going through the installation guide for the RAK3172 WisDuo LoRaWAN Module, ensure you have prepared the necessary items listed below.

- <a href="https://store.rakwireless.com/products/wisduo-lpwan-module-rak3172?utm_source=WisBlockRAK3172&utm_medium=Document&utm_campaign=BuyFromStore" target="_blank">RAK3172 WisDuo LoRaWAN Module</a>
- Computer
...
```
After that, add **RAKwireless RUI STM32 Boards** via the Arduino board manager.
...
The RAK3172 requires a few hardware connections before it can work. The minimum requirement is a properly configured power section, reset button, antenna, and USB connection.

:::warning
Firmware updates are done via UART2 pins. If you connect the module to an external device interfacing with UART2, take extra precautions in your board design to ensure you can still perform firmware updates.  Your board design should include a way to disconnect the external device from the RAK3172 UART2 before connecting the module to a PC (via a USB-UART converter) for the firmware update process.
...
If you don't have an Arduino IDE yet, you can download it on the <a href="https://www.arduino.cc/en/Main/Software" target="_blank">Arduino official website</a> and follow the installation procedure in the [miscellaneous section](#arduino-installation) of this document.
...
Once the Arduino IDE is successfully installed, you can configure the IDE to add the RAK3172 to its board selection by following these steps.
...
:::tip NOTE
The RAK3172 should automatically enter BOOT mode when the firmware is uploaded via Arduino IDE.
...
This example illustrates how to program RAK3172 module as a stand-alone LoRaWAN end-device via <a href="https://docs.rakwireless.com/product-categories/software-apis-and-libraries/rui3/arduino-api/" target="_blank">RUI3 Arduino APIs</a>. To use RAK3172 module as a LoRaWAN end-device, it needs to be within reach of a working **LoRaWAN gateway** registered to a **LoRaWAN network server (LNS)** or one with a built-in network server.
...
To run this example correctly, configure the LoRaWAN parameters and create an OTAA application on your LoRaWAN gateway.
...
###### Upload the LoRaWAN Example to RAK3172
...
This guide uses the US915 regional band, so you need to update the band in the code as well. In addition, you also need to set up the channel mask (channels 8 to 15 are the most commonly used channels in the US915 band).
...
- Ensure you have configured the correct RAK board before uploading the code. See [Configure RAK3172 on Boards Manager](#configure-rak3172-on-boards-manager) section.
...
The RAK3172 module can be configured using AT commands via the UART2 interface.  A USB-to-UART TTL adapter is needed to connect the RAK3172 to your computer's USB port, along with a serial terminal tool. You can use the <a href="https://downloads.rakwireless.com/LoRa/Tools/RAK_SERIAL_PORT_TOOL_V1.2.1.zip" target="_blank">RAK Serial Port Tool</a> so you can easily send AT commands and view the replies from the console output. The RAK Serial Port Tool commands still use the RUI V2 AT commands by default. You can modify it to use RUI3 AT commands and then save it.

:::warning
Firmware updates and AT command functionality are done via UART2 pins. If you connect the module to an external host MCU that sends AT commands via UART2, take extra precautions in your board design to ensure you can still perform firmware updates.  Your board design should include a way to disconnect the host MCU UART to connect to the RAK3172 UART2 before connecting the module to a PC (via a USB-UART converter) for the firmware update process.

An alternative firmware update method, aside from UART2, is to use SWD pins (SWCLK & SWDIO). This method requires external tools like ST-LINK and RAKDAP1.
:::
...
To enable the RAK3172 module as a LoRa P2P module or LoRaWAN end-device, it must be configured by sending AT commands.  You can configure the RAK3172 in two ways:

- [LoRaWAN End-Device](#configure-rak3172-as-lorawan-end-device) - RAK3172 as LoRaWAN IoT device.
- [LoRa P2P](#lora-p2p-mode) - Point-to-point communication between two RAK3172 modules.

#### Configure RAK3172 as LoRaWAN End-Device

To enable the RAK3172 module as a LoRaWAN end device, a device must first be registered in the LoRaWAN network server. This guide covers both TTN and ChirpStack LoRaWAN network servers and the associated AT commands for the RAK3172.

This guide covers the following topics:

- [TheThingsNetwork Guide](#connect-with-the-things-network-ttn) - How to log in, register new accounts and create new applications on TTN.
...
This section provides a quick tutorial on connecting the RAK3172 module to the TTN platform.

:::tip NOTE

In this guide, you will need a working gateway connected to TTN, or you must be within the coverage of a TTN community network.
...
To configure the masking of channels for the sub-bands, you can use the <a href="https://docs.rakwireless.com/product-categories/wisduo/rak3172-module/at-command-manual/#atmask" target="_blank">AT+MASK command</a> that can be found on the AT Commands Manual.
...
If you want to upgrade to the latest version of the firmware of the module, you can follow this section. The latest firmware can be found in the software section of <a href="https://docs.rakwireless.com/product-categories/wisduo/rak3172-module/datasheet/#firmware" target="_blank">RAK3172 Datasheet</a>.
...
1.  Download the latest application firmware of the RAK3172.
    ...
    - <a href="https://downloads.rakwireless.com/#RUI/RUI3/Image/" target="_blank">RAK3172 Firmware</a>

You can also refer to the table below:

|      Model       |                             Version                              |                                             Source                                                                         |
| :--------------: | :--------------------------------------------------------------: | :------------------------------------------------------------------------------------------------------------------------: |
|  RAK3172 (.bin)  |      RUI3 Application Code only (default baudrate = 115200)      |  <a href="https://downloads.rakwireless.com/RUI/RUI3/Image/RAK3172-E_latest.bin" target="_blank">Download</a>              |
|  RAK3172 (.hex)  | RUI3 Bootloader and Application Code(default baudrate = 115200)  |  <a href="https://downloads.rakwireless.com/RUI/RUI3/Image/RAK3172-E_latest_final.hex" target="_blank">Download</a>        |
| RAK3172-T (.bin) |      RUI3 Application Code only(default baudrate = 115200)       |  <a href="https://downloads.rakwireless.com/RUI/RUI3/Image/RAK3172-T_latest.bin" target="_blank">Download</a>              |
| RAK3172-T (.hex) | RUI3 Bootloader and Application Code (default baudrate = 115200) |  <a href="https://downloads.rakwireless.com/RUI/RUI3/Image/RAK3172-T_latest_final.hex" target="_blank">Download</a>        |
|  RAK3172 (OLD)   |         **DEPRECATED** V1.0.4 (default baudrate = 9600)          |  <a href="https://downloads.rakwireless.com/LoRa/RAK3172/Firmware/RAK3172_Latest_Firmware.zip" target="_blank">Download</a>|
