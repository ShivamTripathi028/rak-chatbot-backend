...
You can download the CE and FCC certification guides as PDF files, along with the COMTool serial terminal application, from the [RUI3 section of our Download Center](https://downloads.rakwireless.com/#RUI/RUI3/).

The latest RUI3 firmware versions are available in the [Image folder](https://downloads.rakwireless.com/#RUI/RUI3/Image/) within the same section.
...
| Occupied Bandwidth  | LoRa 125 kHz / 250 kHz 
 FSK 50 kbps (~100 kHz)                                                                                                                  |

#### Power Limit Requirements
| Parameters         | Specification                                                               |
|--------------------|-----------------------------------------------------------------------------|
| Max Transmit Power | Band P at 869.525 MHz≤ 27 dBm 
 Other channels: < 14 dBm |
| Test Signal        | Continuous transmission (LoRa/FSK), DM-2 signal                             |

#### Test Bands & Channel List

**LoRa 125 kHz / FSK Channel List**
...
> **Image:** Hardware connection diagram

**Devices with USB Connection**
...
> **Image:** Hardware connection for devices with USB connection
...
| 2. Configure LoRa parameters             | `AT+P2P=<Frequency>:<SF>:<Bandwidth>:<CR>:<Preamble>:<Power>` 

 **Example:**
`AT+P2P=868000000:7:125:0:65535:14` | <ul><li>**Frequency:** 863–870 MHz </li><li> **SF:** 7-12 (When using SF7, check if the **Timed Send** is set to 70000 ms to maintain a long continuous signal.) </li><li> **Bandwidth:** 125, 250, or 500 </li><li> **CR:** 0 = 4/5, 1 = 4/6, 2 = 4/7, 3 = 4/8 </li><li> **Preamble:** 2-65535 </li><li> **Power:** 5-22 dBm </li></ul> |
| 3. Start transmission (sends a waveform) | `AT+PSEND=<payload>` 
...
> **Image:** LoRa Test Waveform

#### CW Command (Frequency Accuracy Test)

| Function                                                                                                                       | AT Command                                                                                        | Parameters                                                                                                                                   |
|--------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------|
| Starts continuous-wave transmission for PPM frequency accuracy testing. A single command is sufficient to enter the test mode. | `AT+CW=<Frequency>:<Power>:<Duration>` 

  **Example:** 
 `AT+CW=868000000:20:500` | <ul><li> **Frequency:** 863-870 MHz </li><li> **Power:** 5-22 dBm </li><li> **Duration:** signal length in milliseconds </li></ul> |

**CW Test Waveform**

> **Image:** CW Test Waveform
...
> **Image:** FSK Test Setup

**FSK Test Waveform**

> **Image:** FSK Test Waveform

#### Stop Transmission Command

If timed send is enabled, remember to disable it. Use this command:
 ...
 **Example:** 
 `AT+P2P=902300000:7:125:0:65535:22` | <ul><li> **Frequency:** 902-928 MHz </li><li> **SF:** 7-12 (When using SF7, check if the **Timed Send** is set to 70000 ms to maintain a long continuous signal.) </li><li> **Bandwidth:** 125, 250, or 500  </li><li> **CR:** 0 = 4/5, 1 = 4/6, 2 = 4/7, 3 = 4/8 </li><li> **Preamble:** 2 to 65535 </li><li> **Power:** 5-22 dBm </li></ul> |
| 3. Start transmission (begin testing) | `AT+PSEND=<payload>` 
...
| 6：Configure and start hopping transmission | `AT+TRTH=<start_freq>:<end_freq>:<hop_step>:<hop_count>` 
...
> **Image:** Hopping Waveform

#### Stop RF

To stop RF transmission, send the following command:
