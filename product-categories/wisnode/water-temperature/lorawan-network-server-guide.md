---
slug: /product-categories/wisnode/water-temperature/lorawan-network-server-guide/
title: Water Temperature Monitoring Solution LoRaWAN Network Server Guide
...
The cloud network server deployment solution connects the gateways and devices to a third-party cloud network server. This server integrates visualization applications to manage water temperature sensor data in real time.
...
Modify the parameter values corresponding to the topics based on the actual application created and the device used. Then click the **Add** button to save and complete the settings.
...
- **Activation Mode**: Select the activation mode of the device: OTAA or ABP.
  - Choosing the ABP mode will pop up two additional fields, **Application Session Key** and **Network Session Key**. 
...
To get started, you can create an account on the <a href="https://datacake.co/" target="_blank">official website</a> and log in.
...
1. On the device details page, navigate to the **Dashboard** tab, then toggle on the edit mode switch (<img src="https://images.docs.rakwireless.com/wisnode/water-temperature/lorawan-network-server/65-a.edit-switch.png" alt="Edit switch" width="18px"/>).
